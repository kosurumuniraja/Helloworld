...
read -s password
presentdate=`date +'%s'`
list_of_branches=$(curl -s -u $user_name:$password https://api.github.com/repos/kosurumuiraja/Helloworld/branches | jq '.[].name')
for branch in $list_of_branches; do 
api_branch_name=$(echo $branch | cut -d'"' -f 2)
last_updated_date=$(curl -s -u $user_name:$password https://api.github.com/repos/kosurumuiraja/Helloworld/branches/master | jq '.commit.commit.author.date')
